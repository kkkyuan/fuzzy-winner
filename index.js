...
function getWeather(citycode){
    myAxios({
      ...
      <ul class="sun-list">
        ...
        const dayF = dayForecast.map(item => {
        ...
        document.querySelector('.week-wrap').innerHTML = dayF
    })
}
getWeather('510100') 
...
document.querySelector('.search-city').addEventListener('input',(e) => {
    myAxios({
        ...
        const liSrt = result.data.map(item => {
            return `<li class="city-item" data-code="${item.code}">${item.name}</li>`
        }).join('')
        document.querySelector('.search-list').innerHTML = liSrt
    })
})
